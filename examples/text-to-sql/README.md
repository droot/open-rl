...
*   Calls our OpenRL server (API server) to request samples from vLLM.
...
This sequence diagram illustrates the interaction between the client script, the API server/trainer, and the vLLM sampler during the RL training phase, noting the key functions called.

```mermaid
sequenceDiagram
    participant Client as Client Script<br/>(texttosql_sft_grpo.py)
    participant APIServer as API server / Trainer<br/>(GPU 1)
    participant Sampler as vLLM Sampler<br/>(GPU 0)

    Note over Client,Sampler: 1. Rollout Phase
    Client->>APIServer: 1.1 Save current weights for sampler<br/>(save_weights_and_get_sampling_client_async)
    APIServer-->>Client: Acknowledged
    Client->>APIServer: 1.2 Request N samples for prompt<br/>(sample_async)
    APIServer->>Sampler: 1.3 Forward generation request (API)
    Sampler-->>APIServer: 1.4 Return generated SQL completions
    APIServer-->>Client: Return samples to client
    ...
    Client->>APIServer: 3.1 Send data for Forward/Backward (Grads)<br/>(forward_backward_async)
    APIServer->>APIServer: Compute loss & gradients on GPU 1
    APIServer-->>Client: Acknowledged
    Client->>APIServer: 3.2 Send request to step optimizer<br/>(optim_step_async)
    APIServer->>APIServer: Update weights on GPU 1
    APIServer-->>Client: Acknowledged
```
</details>
...
```bash
# OpenRL API server URL
export TINKER_BASE_URL=http://127.0.0.1:9003

# Dummy API key for local API server
