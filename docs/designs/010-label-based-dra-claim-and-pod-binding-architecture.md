...
3. **Least-Privilege RBAC Architecture**: The API server ServiceAccount (`open-rl-sa`) only requires **read-only (`get`, `list`, `watch`)** access to DRA `ResourceClaim` objects. Bound model leases are tracked on **active Pod labels** (`open-rl.io/assigned-claim`, `open-rl.io/bound-base-model`), eliminating the need for elevated write/patch permissions on cluster DRA resources.
