# Design Doc 002: API server Model Configuration & Metadata Refactor
...
This design document specifies the refactoring of model configuration retrieval, metadata persistence, and worker launching across `src/server/api_server.py`, `src/server/worker_manager.py`, and `src/server/k8s_worker_manager.py`.
...
Instead of passing `base_model`, `weight_sync_strategy`, and `training_kind` through function signatures across API server helpers and worker managers:
...
- **Primary Test Files**:
  ...
  - `tests/test_api_server_paths.py`
...
- **Functions Added**:
  - `_extract_and_persist_model_metadata()` (Helper in `api_server.py`)
...
#### Updated Dataclass & New Helper Implementation (`src/server/api_server.py`)
```python
@dataclass
...
#### Refactored `create_model` (`src/server/api_server.py`)
```python
...
#### Refactored `create_model_from_state` (`src/server/api_server.py`)
```python
...
#### Refactored `launch_worker_and_enqueue` (`src/server/api_server.py`)
```diff
...
#### Refactored `ensure_sampler_launched` (`src/server/api_server.py`)
```diff
...
### Opportunity 4: Eliminate Dual-Key Storage in `delete_model` (`src/server/api_server.py`)
...
| :--- | :--- | :--- | :---: |
| `TrainingModelMetadata` | `src/server/api_server.py` | **Refactored** — Made `base_model` optional (`str | None`) so metadata is always written upon creation/restoration. | `0` |
| `_extract_and_persist_model_metadata` | `src/server/api_server.py` | **New Function** — Centralizes validation, header parsing (`x-open-rl-weight-sync-strategy` / `x-open-rl-training-kind`), and `open_rl:model_meta:<model_id>` storage. | `+35` |
| `create_model` | `src/server/api_server.py` | **Refactored** — Replaced inline metadata extraction/storage and dual-key writing with helper call. | `-26` |
| `create_model_from_state` | `src/server/api_server.py` | **Refactored / Fix** — Replaced inline metadata extraction with helper call and included `base_model` / `full_config` in `make_training_request` payload. | `-6` |
| `launch_worker_and_enqueue` | `src/server/api_server.py` | **Refactored / Fix** — Removed destructive `model_meta` overwriting and `model_base` writing; simplified to take/pass only `model_id`. | `-14` |
| `ensure_sampler_launched` | `src/server/api_server.py` | **Refactored** — Removed legacy fallback reads for `open_rl:model_base:<id>`, string type checks, and extra arguments; simplified to take `model_id`. | `-18` |
| `delete_model` | `src/server/api_server.py` | **Refactored** — Removed deletion of `open_rl:model_base:<id>`. | `0` |
...
1. **Disconnected Stubs & Mocking Blind Spots (`test_worker_manager.py`)**: `ApiServerInlineWorkerLaunchTest` mocks out `fft_worker_manager` with `WorkerManagerStub` (which records `launched_model_ids` without checking Redis metadata). Meanwhile, `FFTWorkerManagerTest` calls `manager.launch("Model_A.1")` directly without calling `create_model` first. Because of this separation, **the contract between the API server writing `open_rl:model_meta:<id>` and the worker manager retrieving it from Redis is never tested end-to-end.**
...
3. **Broken Test Stub (`StoreStub.get_value_sync`)**: Running `tests/test_worker_manager.py` crashes in `ApiServerInlineWorkerLaunchTest.test_create_model_launches_worker_then_enqueues` with `AttributeError: 'StoreStub' object has no attribute 'get_value_sync'`. That is because `test_worker_manager.py:L81` calls `self.store.get_value_sync(...)`, but `StoreStub` only implemented `async def get_value(...)`.
...
Add `get_value_sync` to `StoreStub` to prevent inline API server tests from crashing with `AttributeError`:
```python
class StoreStub:
  ...
...
async def test_create_model_from_state_launches_worker_then_enqueues(self) -> None:
  with patch.dict("os.environ", {"OPEN_RL_ENABLE_FFT": "true"}):
    result = await API server.create_model_from_state({"state_path": "/tmp/checkpoint", "base_model": "restored-base", "weight_sync_strategy": "delta"})
...
async def test_ensure_sampler_launched_delegates_to_worker_manager_with_model_id(self) -> None:
  ...
  with patch.dict("os.environ", {"OPEN_RL_ENABLE_FFT": "true", "SAMPLING_BACKEND": "vllm"}):
    ...
    await API server.ensure_sampler_launched("model-x")
...
1. **Local Port-Forward / Fast E2E Smoke Test**:
   ...
   ```
   *Verify in the API server logs that trainer and vLLM sampler dynamic workers boot using `open_rl:model_meta:<id>` and that no `open_rl:model_base:<id>` keys are created in Redis.*
2. **Distributed Kubernetes E2E Benchmark**:
   ...
   ```
   *Inspect the live progression table via `metrics.jsonl` inside the API server pod to confirm clean step execution and dynamic pod orchestration.*
